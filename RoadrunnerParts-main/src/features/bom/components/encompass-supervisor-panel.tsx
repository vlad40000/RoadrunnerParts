...
export function EncompassSupervisorPanel({ jobId, model, onTruthCaptured }: EncompassSupervisorProps) {
  ...
  async function handleCapture() {
    try {
      if (!jobId) {
        throw new Error("Create or load a BOM job before queuing Encompass retrieval.");
      }

      setLoading(true);
      const res = await fetch(`/api/bom/jobs/${jobId}/encompass-retrieval`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model }),
      });

      if (!res.ok) throw new Error("Encompass queue failed");
      
      const data = await res.json();
      const normalizedTruth = {
        ...data,
        status: data.status || "queued",
        canonUrl: data.retrievalJob?.sourceUrl || null,
        retrievalJobId: data.retrievalJob?.id || null,
      };
      ...
      console.error(err);
      alert(err instanceof Error ? err.message : "Encompass queue failed.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <span className="text-xs font-medium text-neutral-600 group-hover:text-blue-700">
              {loading ? "Queuing..." : "Queue Encompass"}
            </span>
          </button>
