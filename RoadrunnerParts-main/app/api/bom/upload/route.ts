...
import { prepareManualIdentityContext } from "@/src/features/bom/services/manual/manual-ingest";
    ...
    if (shouldRunIdentity && identityFiles.length > 0) {
      ...
      if (uploadedIdentity?.url) {
        try {
          ...
          const isPdf =
            uploadedIdentity.mimeType === "application/pdf" ||
            String(uploadedIdentity.originalName || uploadedIdentity.fileName || uploadedIdentity.name || "")
              .toLowerCase()
              .endsWith(".pdf");

          let evidenceText: string | undefined;
          const identityEvidenceFiles = [
            {
          ...
          if (isPdf) {
            try {
              const manualContext = await prepareManualIdentityContext({
                uri: uploadedIdentity.url,
                fileName: uploadedIdentity.originalName || uploadedIdentity.fileName || uploadedIdentity.name,
              });
              evidenceText = manualContext.markdown;
              identityEvidenceFiles.length = 0;
            } catch (error) {
          ...
          await updateBomJobSummary(job.id, {
            ...
            productType: identity.product_type,
          });
        } catch (error) {
          identityError =
