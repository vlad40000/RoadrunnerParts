...
async function bootstrap() {
  ...
  try {
    ...
    `;
    await sql`CREATE INDEX IF NOT EXISTS bom_retrieval_jobs_status_idx ON bom_retrieval_jobs (status, priority, created_at);`;
    await sql`CREATE INDEX IF NOT EXISTS bom_retrieval_jobs_bom_job_idx ON bom_retrieval_jobs (bom_job_id);`;
    await sql`CREATE INDEX IF NOT EXISTS bom_retrieval_jobs_provider_model_idx ON bom_retrieval_jobs (provider, model);`;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS bom_retrieval_jobs_dedup_idx ON bom_retrieval_jobs (bom_job_id, provider, job_type, model);`;

    console.log(' ✓ bom_retrieval_jobs');
    ...
    `;
    await sql`ALTER TABLE retrieval_jobs ADD COLUMN IF NOT EXISTS bom_job_id text;`;
    await sql`ALTER TABLE retrieval_jobs ADD COLUMN IF NOT EXISTS source_url_id uuid REFERENCES model_source_urls(id) ON DELETE SET NULL;`;
    await sql`ALTER TABLE retrieval_jobs ADD COLUMN IF NOT EXISTS result_summary jsonb NOT NULL DEFAULT '{}'::jsonb;`;
    await sql`ALTER TABLE retrieval_jobs ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();`;
