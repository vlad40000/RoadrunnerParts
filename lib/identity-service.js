...
/**
 * Normalizes input identifiers by stripping whitespace and noise, but PRESERVING internal slashes
 * if they are part of a common model suffix pattern (like Samsung WA45H7000AW/A2).
 */
export function normalizeIdentifier(val) {
  if (!val || typeof val !== 'string') return '';
  // Basic trim and uppercase
  const base = val.trim().toUpperCase();
  // Remove spaces, dots, and backslashes, but leave forward slashes and hyphens for now
  return base.replace(/[\s\.\\]+/g, '');
}

/**
 * Strips all non-alphanumeric noise completely (Legacy/Strict mode).
 */
export function strictNormalize(val) {
  return normalizeIdentifier(val).replace(/[^A-Z0-9]/g, '');
}

/**
 * Generates an array of model candidates for lookup.
 * Includes: raw, strict-normalized, and slash-stripped variants.
 */
export function getLookupCandidates(rawModel) {
  if (!rawModel) return [];
  ...
  // If it has a slash, add the version BEFORE the slash (family root)
