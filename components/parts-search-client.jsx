              ...
              <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-3 rounded-xl border border-amber-100 bg-amber-50 px-3 py-2">
                    ...
                    <ul className="mt-1 list-inside list-disc space-y-1">
                      {results.manualReviewFlags.map((flag, i) => (
                        <li key={i} className="text-[10px] font-medium text-amber-700">{flag}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {results?.blockers?.length > 0 && (
                  <div className="mt-3 rounded-xl border border-red-100 bg-red-50 px-3 py-2">
                    <div className="text-[10px] font-black uppercase text-red-800 tracking-wider">Blockers</div>
                    <ul className="mt-1 list-inside list-disc space-y-1">
                      {results.blockers.map((blocker, i) => (
                        <li key={i} className="text-[10px] font-medium text-red-700">{blocker}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {results?.issues?.length > 0 && (
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-2">
                      Extraction Issues & Flags
                    </div>
                    <ul className="space-y-1">
                      {results.issues.map((issue, idx) => (
                        <li key={idx} className="text-[10px] font-bold text-amber-800 flex items-start gap-2">
                ...
                return (
                  <>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2 mb-4 text-xs">
                      ...
                      {results.sourceStrategy && (
                        <span className="flex items-center gap-1.5 rounded-full bg-indigo-50 px-3 py-1 text-indigo-700 ring-1 ring-indigo-100 font-bold">
                          <Settings size={12} /> Strategy: {results.sourceStrategy}
                        </span>
                      )}
                      {results.truthSource && (
                        <span className="flex items-center gap-1.5 rounded-full bg-purple-50 px-3 py-1 text-purple-700 ring-1 ring-purple-100 font-bold">
                          <Info size={12} /> Origin: {results.truthSource}
                        </span>
                      )}
                            ...
                            <table className="w-full text-left text-sm text-slate-600 border-collapse">
                              <thead className="bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                <tr>
                                  ...
                                  <th className="px-6 py-4 border-b border-slate-200">eBay</th>
                                      ...
                                      {getBomRowPartNumber(part) ? (
                                        <div className="flex flex-col gap-2">
                                          <a
                                            ...
                                            className="inline-flex w-fit items-center rounded-lg border border-blue-200 bg-white px-3 py-1.5 text-[11px] font-bold text-blue-700 hover:bg-blue-50"
                                          >
                                            Search eBay
                                          </a>
                                          <a
                                            ...
                                            rel="noreferrer"
                                            onClick={(event) => event.stopPropagation()}
                                            className="inline-flex w-fit items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50"
                                          >
                                            Sold Comps
                                          </a>
                                        </div>
...
function getResultsFromJob(job) {
  ...
  return {
    ...
    sourceStrategy: job?.sourceStrategy || null,
    truthSource: job?.truthSource || null,
  };
}
