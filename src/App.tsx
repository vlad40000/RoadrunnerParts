...
- "modelMSRP" (number, optional if high confidence only).`,
  ...
  const handleSubmitReview = async () => {
    if (!user || !selectedPart || !newComment) return;
    ...
    try {
      await submitReview({
        ...
        status: isInstalled ? 'installed' : 'used'
      });
      setNewComment('');
      loadPartData(selectedPart.partNumber);
    } catch (error) {
      console.error(error);
      alert("Failed to submit review. Ensure your email is verified.");
    } finally {
                        ...
                        {user ? (
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  ...
                                  >
                                    <Star size={20} className={newRating >= star ? 'fill-amber-400 text-amber-400' : 'text-pro-slate-200'} />
                                  </button>
                                ))}
                              </div>
                              <button 
                                ...
                                className={`text-[10px] font-bold px-3 py-1 rounded transition-colors ${
                                  isInstalled ? 'bg-pro-blue text-white' : 'bg-pro-slate-200 text-pro-slate-500'
                                }`}
                              >
                                {isInstalled ? 'VERIFIED INSTALL' : 'DEMO/USED'}
                              </button>
                            </div>
                             <div className="relative group">
                              <textarea 
                                placeholder="ASK ASSISTANT OR DESCRIBE OBSERVATIONS..."
                                className="pro-input w-full p-4 pr-24 text-xs min-h-[120px] bg-white border border-pro-slate-200 focus:border-pro-blue shadow-sm"
                                value={newComment}
                                onChange={(e) => setNewComment(e.target.value)}
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' && e.metaKey) handleFieldAIChat(newComment);
                                }}
                              />
                              <div className="absolute bottom-3 right-3 flex gap-2">
                                <button 
                                  ...
                                  className={`p-2.5 rounded-xl transition-all shadow-sm ${
                                    isRecording 
                                      ...
                                      : 'bg-pro-slate-100 text-pro-slate-400 hover:text-pro-blue hover:bg-pro-blue/5'
                                  }`}
                                  title={isRecording ? "Stop Local Capture" : "Capture Voice Note"}
                                >
                                ...
                                <button 
                                  type="button"
                                  onClick={() => {
                                    handleFieldAIChat(newComment);
                                    setNewComment('');
                                  }}
                                  disabled={!newComment || isFieldChatLoading}
                                  className="p-2.5 bg-pro-navy text-white rounded-xl hover:bg-pro-blue transition-all disabled:opacity-30 shadow-sm"
                                  title="Sync to AI Logic"
                                >
                                  <Send size={18} />
                                </button>
                              </div>
                            ...
                            <button 
                              ...
                              className="w-full pro-button pro-button-primary py-3 shadow-pro-md flex items-center justify-center gap-2 group"
                            >
                              {isSubmitting ? <Loader2 size={14} className="animate-spin" /> : (
                                <>
                                  <ClipboardList size={16} className="group-hover:rotate-12 transition-transform" />
                                  <span>Commit Technical Log</span>
                                </>
                              )}
                            </button>
                          </div>
                        ) : (
                          <div onClick={signIn} className="pro-card border-dashed p-6 flex flex-col items-center gap-4 cursor-pointer hover:bg-pro-slate-100 transition-all group rounded-xl">
                            <User className="text-pro-slate-300 group-hover:text-pro-blue transition-colors" size={24} />
                            <p className="text-[10px] font-bold uppercase tracking-widest text-pro-slate-400">Initialize session to submit logs</p>
                          </div>
                        )}
                      </div>
                    </div>
