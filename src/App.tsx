      ...
      <div className="flex flex-col gap-4 border-b border-pro-slate-200 bg-white px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <a
              href="/ebay"
              className="inline-flex h-9 items-center rounded-md border border-pro-navy bg-pro-navy px-3 text-[10px] font-black uppercase tracking-[0.16em] text-white transition-colors hover:bg-pro-blue"
            >
              Open batch dashboard
            </a>
            <a
              href="/ebay_mockup_gallery.html"
              className="inline-flex h-9 items-center rounded-md border border-pro-blue bg-white px-3 text-[10px] font-black uppercase tracking-[0.16em] text-pro-blue transition-colors hover:bg-pro-blue hover:text-white"
            >
              Open live mockups
            </a>
          </div>
            ...
            {readyItems.map((item) => (
              <article key={item.partNumber} className="rounded-lg border border-pro-slate-200 bg-white p-3 shadow-sm">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  <a
                    href={`/ebay/${encodeURIComponent(item.partNumber)}`}
                    className="rounded-md border border-pro-blue bg-pro-blue px-2 py-2 text-center text-[10px] font-black uppercase tracking-wider text-white hover:bg-pro-navy"
                  >
                    Editor
                  </a>
                  <a
          ...
          <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
            {visiblePendingItems.map((item) => (
              <a
                key={item.partNumber}
                href={`/ebay/${encodeURIComponent(item.partNumber)}`}
                className="block rounded-md border border-amber-200 bg-white px-3 py-2 transition-colors hover:border-amber-400 hover:bg-amber-50"
              >
                ...
                <span className="mt-2 inline-flex rounded border border-amber-200 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-amber-700">
                  Open editor
                </span>
              </a>
            ))}
...
export default function App({ currentEbayBatch = null }: { currentEbayBatch?: CurrentEbayBatch | null }) {
  ...
  const getBomPassCopy = (query: string, isExhaustive: boolean, passNumber: number) => {
    if (!isExhaustive) {
      return {
...
Focus on speed and reliability for the most common serviceable parts.`,
    ...
    const promptTitle = `Generate an ABSOLUTELY EXHAUSTIVE, MASTER-LEVEL Bill of Materials (BOM) for appliance model: ${query}.`;
...
- screws, bolts, and small service hardware`,
  ...
  const handleAILookup = async (modelToSearch?: string, serialToSearch?: string, isExhaustive = false) => {
    ...
    const passNumber = knownPartNumbers.length === 0
      ? 1
      : isExhaustive
        ? (bomPassCount > 0 ? bomPassCount + 1 : 1)
        : 1;
    const { passInstruction } = getBomPassCopy(normalizedQuery, isExhaustive, passNumber);
    ...
    await runApprovedBomPrompt(requestPayload);
  };
  ...
  const runApprovedBomPrompt = async (requestPayload: any, promptText?: string) => {
    ...
    try {
      const response = await fetch('/api/bom', {
        ...
        body: JSON.stringify({
          ...requestPayload,
          ...(promptText?.trim() ? { promptOverride: promptText } : {}),
        }),
      });
      ...
      // Generate stable display IDs based on sorted results. Keep unpriced source-backed rows visible.
  ...
  }, [searchTerm, selectedSectionKeys, aiParts, sortBy, hasModelContext]);
