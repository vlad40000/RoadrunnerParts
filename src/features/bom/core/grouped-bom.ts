...
import { partSelectDiagramsProvider } from "../services/providers/partselect-diagrams";
...
function toRawReconcileRows(group: { groupName: string; sourceUrl: string }, rows: any[] = []) {
  return rows.map((row) => ({
...
function parseStructuredGroupRows(text: string | null | undefined) {
  if (!text) return [];
...
export async function discoverDiagramGroupsForJob(input: {
  ...
  await updateBomJobSummary(input.jobId, {
    ...
    resultStatus: job.resultStatus ?? "identity_only",
  });
  ...
  let usedProvider = "sears-partsdirect";

  // Try Sears first
  providerSources = await searsPartsDirectProvider.fetchSources({
    model,
    brand: input.identity.brand ?? job.brand ?? null,
    productType: input.identity.productType ?? job.productType ?? null,
  } as any);

  // Fallback to PartSelect if no sources found
  if (!providerSources.length) {
    usedProvider = "partselect-diagrams";
    providerSources = await partSelectDiagramsProvider.fetchSources({
      model,
      brand: input.identity.brand ?? job.brand ?? null,
      productType: input.identity.productType ?? job.productType ?? null,
    } as any);
  ...
  if (!providerSources.length) {
    ...
    throw new Error(`No diagram groups were found for ${model} on Sears or PartSelect.`);
  }
  ...
  if (meta.expectedPartsTotal) {
    await updateBomJobSummary(input.jobId, {
      ...
      expectedPartsConfidence: meta.expectedPartsConfidence || 0.95,
    } as any);
  }
...
export async function extractDiagramGroupForJob(input: {
  ...
  try {
    const diagramRows = parseStructuredGroupRows(group.sourceText).map((row) => ({
      ...row,
      source_url: group.sourceUrl,
      diagram_group: group.groupName,
    }));
    ...
    const optimized = buildOptimizedResponse(
      ...
      {
        summary: `Grouped Sears BOM for ${job.model || (job.identity as any)?.model || ""}`,
        brand: job.brand,
        ...
        truthSource: "Sears diagram groups",
  ...
  await runWithConcurrency(pendingGroups, concurrency, async (group) => {
    try {
      ...
      if (!sourceText || sourceText.includes("(Diagram groups are typically extracted sequentially)")) {
        ...
        // Special case for PartSelect rows if we are in this fallback
        if (group.source === "partselect-diagrams") {
          const $ = load(html);
          const psRows: any[] = [];
      ...
      let diagramRows = parseStructuredGroupRows(sourceText).map((row) => ({
        ...row,
