...
export async function runGroundedAiAgent(input: {
  ...
  const prompt = `Generate an EXHAUSTIVE AND COMPLETE Bill of Materials (BOM) for: ${input.brand} model ${input.model}. 
    ...
    Aim for 100-200 parts for major appliances. Use REAL OEM part numbers where possible. 
    Search specifically for "Search ${input.model} parts list diagrams" on searspartsdirect.com, repairclinic.com, and partselect.com.
