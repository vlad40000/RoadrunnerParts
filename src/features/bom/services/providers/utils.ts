...
function isLikelyHtmlErrorPage(body: string) {
  ...
  return (
    /<title>\s*(?:500|internal server error|application error|bad gateway|access denied)/i.test(head) ||
    /<h1[^>]*>\s*(?:500|internal server error|application error|bad gateway|access denied)/i.test(head)
  );
}
