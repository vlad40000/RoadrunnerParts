...
/**
 * Mapping stripped of all OEM adapters. Distributor-only mapping.
 */
const ADAPTER_TO_PROVIDER_NAMES: Record<string, string[]> = {
  ...
  "default": [...DISTRIBUTOR_PRIMARY_PROVIDERS, ...DISTRIBUTOR_SECONDARY_PROVIDERS]
};
...
function filterByTargetSections(sources: RetrievedSource[], targetSections?: string[]) {
  if (!targetSections?.length) return sources;
  const wanted = new Set(targetSections.map((s) => s.trim().toLowerCase()).filter(Boolean));
  return sources.filter((source) => {
    const section = (source.sectionName ?? "").trim().toLowerCase();
    return section ? wanted.has(section) : false;
  });
}
...
function filterProvidersByBrandGate(providers: SourceProvider[], input: { brand: string | null; model: string | null }) {
  ...
  return providers.filter((provider) => approved.has(provider.name as SourceKey));
}

async function runProviders(
  providers: SourceProvider[],
  input: { brand: string | null; model: string | null; productType?: string | null; }
) {
  const model = normalizeModel(input.model);
  if (!model) return [];

  const eligible = providers
    .filter((provider) => provider.supports({ brand: input.brand, model }))
    .sort((a, b) => a.priority - b.priority);

  const merged: RetrievedSource[] = [];
  const providerConcurrency = Math.max(1, Number.parseInt(process.env.BOM_PROVIDER_CONCURRENCY ?? "3", 10) || 3);
  ...
  const providerResults = await runWithConcurrency(
    ...
    async (provider) => {
      try {
        return await provider.fetchSources({ brand: input.brand, model, productType: input.productType ?? null } as any);
      } catch {
        return [];
      }
    },
  ...
  for (const sources of providerResults) {
    if (sources.length) merged.push(...sources);
  }

  return uniqueBy(merged, (s) => `${s.provider}|${s.sectionName}|${s.sourceUrl}`);
}

export function resolveSourceProviderPlan(input: { brand: string | null; model: string | null; }): SourceProviderPlan {
  const model = normalizeModel(input.model);
  const inputBrand = input.brand ?? null;
  const resolvedBrand = model ? resolveTrueOemBrand(input.brand, model) : input.brand ?? null;
  const family = resolvedBrand ? getManufacturerFamilyConfig(resolvedBrand, model ?? "") : null;

  const primaryProviderNames = family?.adapterKey
    ? ADAPTER_TO_PROVIDER_NAMES[family.adapterKey] ?? ADAPTER_TO_PROVIDER_NAMES["default"]
    : ADAPTER_TO_PROVIDER_NAMES["default"];

  const fallbackProviderNames = dedupeStrings([...UNIVERSAL_FALLBACK_PROVIDER_NAMES]);
...
export async function fetchSources(input: {
  ...
  const providerInput = { brand, model, productType: input.productType ?? null };

  // 1. Seeded / User Intake Provider (Top Priority)
  ...
  // 2. Distributor Primary (Encompass, Sears, PartsDr)
  const primaryDistributors = [encompassFamilyProvider, searsPartsDirectProvider, partsDrProvider];
  const primarySources = await runProviders(filterProvidersByBrandGate(primaryDistributors, { brand, model }), providerInput);
  if (primarySources.length > 0) {
    return uniqueBy(primarySources, (s) => `${s.provider}:${s.sourceUrl}`);
  }

  // 3. Distributor Secondary (APP, PartSelect, Fix, RepairClinic)
  const secondaryDistributors = [appliancePartsProsProvider, partSelectProvider, fixComProvider, repairClinicFamilyProvider];
  const secondarySources = await runProviders(filterProvidersByBrandGate(secondaryDistributors, { brand, model }), providerInput);
